...
type
  ...
  public
    ...
    property HidDevices:TList read FList;
...
begin
  ...
  begin
    ...
    Result :=
        ...
        {$else}
        JvHidControllerClass.ReadFileEx(HidOverlappedRead, Report, ToRead, FOvlRead, CallBack);
        {$endif}
  end;
end;
...
begin
  ...
  begin
    ...
    {$else}
    JvHidControllerClass.WriteFileEx(HidOverlappedWrite, Report, ToWrite, FOvlWrite, CallBack);
    {$endif}
...
begin
  ...
  FreeOnTerminate := False;
  Windows.InitializeCriticalSection(FThreadLock);
  SignalDeviceChange := TEventObject.Create(nil,true,false,'');
  FMessageCounter:=0;
end;

destructor TJvHidDeviceControllerMonitorThread.Destroy;
begin
  SignalDeviceChange.Free;
  Windows.DeleteCriticalSection(FThreadLock);
  inherited Destroy;
end;
...
begin
  ...
  while (NOT Terminated) do
  begin
    SignalDeviceChange.WaitFor(INFINITE);
    SignalDeviceChange.ResetEvent;
    ...
    Reps:=InterLockedExchange(FMessageCounter,Reps);
    while (Reps>0) do
    begin
      Dec(Reps);
      if IsLibrary then
...
begin
  ...
  // signal thread !!
  //InterLockedIncrement(FControllerMonitorThread.FMessageCounter);
  FControllerMonitorThread.SignalDeviceChange.SetEvent;
